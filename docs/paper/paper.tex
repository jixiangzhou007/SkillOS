...
Experiments on three skill creation tasks show that SkillOS-generated skills consistently include decision-table routing structures (S\_route) that bare LLM generation never produces (100\% vs 0\%), and that the pipeline achieves differentiated audit scores (70--75/100) validated across 235 benchmark runs with 4,400+ accumulated epistemology claims.
...
Beyond epistemology, we introduce a \textit{domain DNA system} spanning 12 academic disciplines with 13 domain templates and 8 philosophical methods, a \textit{gate semantics} framework ([动作]/[门禁]) that prevents agents from silently skipping critical checks, and a \textit{resource capture pipeline} that extracts scripts, templates, and references from dialogue into AgentSkills.io standard directories.
The complete system represents 82 engineering commits, 605 tests (98.3\% pass), and has accumulated 4,400+ epistemology claims from real pipeline runs.}
\end{abstract}
...
%==============================================================================
\section{Domain DNA: A 12-Discipline Knowledge Framework}
\label{sec:domain-dna}
%==============================================================================

Beyond individual claim verification, SkillOS addresses a second challenge: \textit{domain-aware extraction}. A contract review skill requires knowledge of legal statutes (Civil Code, Company Law); a construction safety skill requires GB standards (GB 50656, JGJ 59); a medical triage skill requires clinical guidelines (NICE, WHO). Without domain context, extraction is generic.

\subsection{Three-Layer Architecture}

SkillOS implements a three-layer domain knowledge system:

\begin{enumerate}[leftmargin=*,nosep]
    \item \textbf{Taxonomy (12 Disciplines).} A keyword-based classification system maps any skill topic to one of 12 academic disciplines (Computer Science, Medicine, Management, Law, Economics, Education, Design, Engineering, Natural Science, Social Science, Journalism, Agriculture). Each discipline carries a \textit{concrete evidence standard} referencing real certification criteria: ISO/IEC 25010 for software quality, JCI for hospital accreditation, IFRS/IAS for financial reporting, WCAG 2.1 AA for accessibility.
    \item \textbf{Methodology Detection (6 Patterns).} Six underlying thinking patterns are detected from process descriptions: engineering (reductionist), scientific (Baconian), diagnostic (Aristotelian), design-thinking (pragmatic), business-process (Taylorist), and creative (deliberate practice). Each methodology carries a philosophical lineage and domain-specific trigger questions.
    \item \textbf{Philosophical DNA (8 Methods).} Eight philosophical methods ground the extraction in intellectual tradition: PDCA (Deming), OODA (Boyd), Scientific Method (Bacon+Popper), Dialectical (Hegel+Marx), Reductionist (Descartes), Pragmatic (Dewey+Simon), Deliberate Practice (Ericsson), and Strategic Adaptive (Sun Tzu+Daoism). Methods map to methodologies, which map to disciplines, which map to 13 domain templates with pre-defined skeletons, trigger conditions, and S\_route structures.
\end{enumerate}

\subsection{Authoritative Reference Registry}

A key practical contribution is a \textit{domain reference library} mapping each of the 12 disciplines to 5--6 authoritative sources (70 total): laws, ISO standards, clinical guidelines, industry certifications. During extraction, detected domain references are automatically injected into the generation prompt, enabling the LLM to reference specific statutory provisions (e.g., ``per GB 50656 Section 4.2'') rather than generic advice.

\subsection{Gate Semantics}

SkillOS introduces a \textit{gate semantics} framework where S\_body steps are explicitly tagged as [动作] (action) or [门禁] (gate). Gate steps define mandatory checkpoints that, if failed, abort or escalate the process---preventing the common agent failure mode of silently skipping critical checks. Gate steps are automatically propagated to the S\_route decision table, ensuring the agent cannot proceed without satisfying all preconditions.
...
Pipeline-generated skills are 2--3$\times$ longer than bare LLM outputs, reflecting the deeper structural decomposition from the seven-step process. The pipeline is approximately 16$\times$ slower (135s vs.\ 8s), consistent with making 7+ LLM calls versus 1. The S\_route decision table---which maps user intents to execution actions and resource references---is the key differentiator: it transforms a static document into an executable agent instruction set.

Across 235 cumulative benchmark runs (accumulated during development of the 82-commit engineering baseline), the pipeline consistently achieves 100\% S\_route coverage while bare LLM generation remains at 0\%. Audit scores are now differentiated (70--75 range) rather than uniformly defaulting to 60, following a JSON parsing robustness improvement that applies four fallback strategies before accepting the default score. Gate semantics ([动作]/[门禁]) automatically propagate 2--4 mandatory checkpoints per skill to the decision table. The epistemology engine has accumulated 4,400+ claims from real pipeline runs, with 2,533 promoted to ``knowledge'' level through cross-referencing.
...
The audit scoring subsystem (10-dimension independent auditor) was initially limited by JSON parsing failures due to LLM response truncation (max\_tokens=800). This was resolved by increasing the token budget to 1500 and adding a four-strategy JSON extraction fallback (code block extraction, raw JSON detection, brace matching, trailing-comma repair). Post-fix benchmarks confirm differentiated scores across skill cases rather than uniform default values.

The epistemic ablation reported above uses offline heuristic falsification; LLM-based falsification may exhibit same-model self-consistency bias. The benchmark includes human labels to ground evaluation, but inter-rater agreement has not yet been measured (single annotator v0). Domain DNA and gate semantics contributions are qualitatively evaluated; quantitative ablation of these components against skill execution quality remains future work.
...
We have presented \textsc{SkillOS}, a system that brings epistemological rigor and domain-aware knowledge engineering to the agent skills ecosystem. By implementing Plato's justified true belief and Popper's falsificationism as engineering primitives, SkillOS provides systematic quality verification that no other skill system offers. The domain DNA framework---spanning 12 academic disciplines, 13 domain templates, 8 philosophical methods, and 70 authoritative reference sources---enables domain-specific extraction with concrete evidence standards. The Socratic extraction pipeline produces structurally complete skills with decision-table routing (100\% S\_route coverage vs.\ 0\% for bare LLM). Gate semantics prevent the common agent failure mode of silently skipping critical checks. The MoE self-evolution system with knowledge diffusion enables collective learning across 4,400+ accumulated epistemology claims.

The Agent Skills ecosystem is projected to grow exponentially through 2026--2027. As it does, the question of skill quality will become increasingly critical---analogous to how software testing became essential as the npm/PyPI ecosystems scaled. SkillOS represents a first step toward treating skill quality as a first-class engineering concern, grounded in philosophical rigor and validated by 235 benchmark runs and 82 engineering iterations.
