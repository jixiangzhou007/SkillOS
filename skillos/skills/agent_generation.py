...
tool_description: <路由规则：用户说什么话触发（如"合同有问题""帮我审一下"），不要写功能列表>
...
## Gotchas（常见坑点）
- <看起来对但实际错的情况，如：返回200不代表成功>
- <容易遗漏的检查点>

## Ecosystem（生态集成建议）
- hooks: <建议配合的 hooks，如 PostToolUse 自动检查结果、Stop 时保存中间产物>
- mcp: <需要接入的 MCP 能力，如 Stripe API、GitHub API>
- subagents: <可以拆分给子代理执行的子任务>
```
...
7. 只输出 skill_doc 代码块
8. Gotchas 必须写"看起来对但实际错"的具体经验（如"返回200不代表成功"），不要写通用常识
9. Skill 是应用层协议，不是提示词增强——应提供中间格式、检查点、工具脚本，而非更长的说明
    ...
    try:
        ...
        if quality_issues:
            ...
            quality_msg = (
                ...
                "- 回复「**生态集成**」帮你分析适合配合什么 hooks、MCP、subagents 使用"
            )
